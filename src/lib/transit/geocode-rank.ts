...
export function hasMappableCoords(lat: number, lon: number) {
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return false;
  if (Math.abs(lat) > 90 || Math.abs(lon) > 180) return false;
  // MOTIS/SAD feeds use -1,-1 when a stop has no geometry.
  if (Math.abs(lat + 1) < 1e-6 && Math.abs(lon + 1) < 1e-6) return false;
  return true;
}

export function isInEurope(lat: number, lon: number) {
  // Transitous covers Europe plus nearby islands (Azores, Canaries, Cyprus, Iceland).
  return lat >= 27.5 && lat <= 71.5 && lon >= -31.5 && lon <= 45;
}

function looksLikeBusStop(match: GeocodeMatch) {
  return /SAD-|JDF-|dpmp_|DPM-|ARRIVA|Arriva|_bus/i.test(match.id);
}
...
export function stopModeKind(
  ...
  const bus =
    modes.some((mode) => mode === "BUS" || mode === "COACH") ||
    looksLikeBusStop(match);
...
function busRank(match: GeocodeMatch, wantRail: boolean) {
  if (match.type !== "STOP") return 4;
...
export function rankGeocodeMatches(
  ...
  query = "",
  options?: { preferType?: GeocodeMatch["type"] },
): GeocodeMatch[] {
  const mappable = matches.filter(
    (match) =>
      hasMappableCoords(match.lat, match.lon) &&
      isInEurope(match.lat, match.lon),
  );
  ...
  if (!trimmed) {
    return [...mappable]
      .sort((a, b) => {
        const byType =
          typeRank(a.type, false, preferType) - typeRank(b.type, false, preferType);
  ...
  return mappable
    ...
    .sort((a, b) => {
      ...
      const byType =
        typeRank(a.match.type, addressQuery, preferType) -
        typeRank(b.match.type, addressQuery, preferType);
      if (byType !== 0) return byType;
      if (
        a.match.type === "STOP" &&
        b.match.type === "STOP" &&
        ...
        const byBus = busRank(a.match, wantRail) - busRank(b.match, wantRail);
